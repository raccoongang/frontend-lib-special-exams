...
describe('Data layer integration tests', () => {
  ...
  describe('Test getAllowProctoringOptOut', () => {
    const getAllowProctoringOptOutUrl = `${getConfig().LMS_BASE_URL}/api/courseware/sequence/${contentId}`;

    it('Should get, and save allowProctoringOptOut', async () => {
      axiosMock.onGet(getAllowProctoringOptOutUrl).reply(200, { allow_proctoring_opt_out: true });

      await executeThunk(thunks.getAllowProctoringOptOut(contentId), store.dispatch);
    ...
    it('Should fail to fetch if error occurs', async () => {
      axiosMock.onGet(getAllowProctoringOptOutUrl).networkError();
  ...
  describe('Test startExam', () => {
    ...
    it('Should start exam, and update attempt and exam', async () => {
      ...
      await executeThunk(thunks.startExam(), store.dispatch, store.getState);
  ...
  describe('Test startProctoringExam', () => {
    ...
    it('Should start exam, and update attempt and exam', async () => {
      ...
      await executeThunk(thunks.startProctoringExam(), store.dispatch, store.getState);
      state = store.getState();
      expect(state.examState.exam.attempt.attempt_status).toBe(ExamStatus.CREATED);
    });
